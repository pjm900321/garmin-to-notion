...
# ---------------------------
def parse_garmin_time_to_kst_iso(dt_str: str) -> str | None:
    """
    Garmin startTimeGMT can be:
      - ISO with Z: 2026-02-01T00:12:34.000Z
      - ISO with offset: 2026-02-01T00:12:34+00:00
      - space-separated: 2026-02-01 00:12:34
    We normalize to KST ISO8601: 2026-02-01T09:12:34+09:00
    """
    if not dt_str:
        return None
    ...
    try:
        ...
        # Try common Garmin "YYYY-MM-DD HH:MM:SS"
        try:
            dt = datetime.strptime(dt_str, "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone.utc)
        except ValueError:
            return None

    # If timezone missing, assume UTC (because it's startTimeGMT)
...
def format_activity_type(activity_type, activity_name=""):
    ...
    activity_type_main = formatted_type
    ...
    if formatted_type == "Rowing V2":
        activity_type_main = "Rowing"
    elif formatted_type in ["Yoga", "Pilates"]:
        activity_type_main = "Yoga/Pilates"
        activity_subtype = formatted_type

    if formatted_type in activity_mapping:
        activity_type_main = activity_mapping[formatted_type]
        activity_subtype = formatted_type
    ...
    return activity_type_main, activity_subtype
...
def format_pace(average_speed):
    if average_speed and average_speed > 0:
        pace_min_km = 1000 / (average_speed * 60)  # min/km
...
# --------------------------------------------------------
# IMPORTANT FIX: dedupe by exact datetime (KST ISO string)
# --------------------------------------------------------
def activity_exists(client, database_id, activity_start_gmt, activity_type, activity_name):
    """
    Dedupe rule:
      - Date (Notion date) must equal the exact KST datetime we store (ISO8601 with +09:00)
      - Activity Type matches
      - Activity Name matches (title equals)
    This prevents duplicates when Date contains time.
    """
    # activity_type might be tuple (main, subtype)
    ...
    lookup_type = "Stretching" if "stretch" in (activity_name or "").lower() else main_type

    start_iso_kst = parse_garmin_time_to_kst_iso(activity_start_gmt)
    if not start_iso_kst:
        return None
    ...
    query = client.databases.query(
        ...
        filter={
            "and": [
                {"property": "Date", "date": {"equals": start_iso_kst}},
...
def activity_needs_update(existing_activity, new_activity):
    ...
    activity_name_raw = new_activity.get("activityName", "")
    activity_name_lower = activity_name_raw.lower()
    ...
    def safe_rich_text(prop_name):
        rt = existing_props.get(prop_name, {}).get("rich_text", [])
        if rt and rt[0].get("text") and "content" in rt[0]["text"]:
            return rt[0]["text"]["content"]
        return ""

    # Some DBs may not have Subactivity Type
    has_subactivity = (
    ...
    return (
        existing_props["Distance (km)"]["number"] != round(new_activity.get("distance", 0) / 1000, 2)
        or existing_props["Duration (min)"]["number"] != round(new_activity.get("duration", 0) / 60, 2)
        or existing_props["Calories"]["number"] != round(new_activity.get("calories", 0))
        or safe_rich_text("Avg Pace") != format_pace(new_activity.get("averageSpeed", 0))
        ...
        or existing_props["Training Effect"]["select"]["name"] != format_training_effect(new_activity.get("trainingEffectLabel", "Unknown"))
        or existing_props["Aerobic"]["number"] != round(new_activity.get("aerobicTrainingEffect", 0), 1)
        or existing_props["Aerobic Effect"]["select"]["name"] != format_training_message(new_activity.get("aerobicTrainingEffectMessage", "Unknown"))
        or existing_props["Anaerobic"]["number"] != round(new_activity.get("anaerobicTrainingEffect", 0), 1)
        or existing_props["Anaerobic Effect"]["select"]["name"] != format_training_message(new_activity.get("anaerobicTrainingEffectMessage", "Unknown"))
        ...
        or (has_subactivity and existing_props["Subactivity Type"]["select"]["name"] != activity_subtype)
        or (not has_subactivity)
    )
...
def create_activity(client, database_id, activity):
    ...
    properties = {
        # ✅ Date is a DATE property, storing datetime (KST ISO) consistently
        "Date": {"date": {"start": start_iso_kst}},
...
def main():
    ...
    if not all([garmin_email, garmin_password, notion_token, database_id]):
        raise ValueError("Missing env vars: GARMIN_EMAIL, GARMIN_PASSWORD, NOTION_TOKEN, NOTION_DB_ID")
    ...
    for activity in activities:
        ...
        # ✅ FIXED: dedupe by exact datetime stored in Date (KST ISO)
        existing_activity = activity_exists(client, database_id, activity_start_gmt, activity_type, activity_name)
